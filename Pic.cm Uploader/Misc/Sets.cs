...
namespace Piccm_Uploader
{
    public static class Sets
    {
        ...
        {
            bool isExists = Directory.Exists(References.APPDATA);
            if (!isExists)
            {
                Directory.CreateDirectory(References.APPDATA);
            }
            if (System.IO.File.Exists(References.APPDATA + "sets") == false)
            {
                System.IO.File.WriteAllBytes(References.APPDATA + "sets", Resources.Resource.default_sets);
            }
            s = new Ini(References.APPDATA + "sets");
